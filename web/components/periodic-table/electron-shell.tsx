"use client"

import { useEffect, useState } from "react"
import { Element, categoryInfo } from "@/lib/elements-data"

interface ElectronShellProps {
  element: Element
}

// 解析电子排布，计算每个壳层的电子数
function getElectronShells(atomicNumber: number): number[] {
  // 电子壳层容量: K=2, L=8, M=18, N=32, O=32, P=18, Q=8
  const shellCapacities = [2, 8, 18, 32, 32, 18, 8]
  const shells: number[] = []
  let remaining = atomicNumber

  for (let i = 0; i < shellCapacities.length && remaining > 0; i++) {
    const electrons = Math.min(remaining, shellCapacities[i])
    shells.push(electrons)
    remaining -= electrons
  }

  // 根据实际元素的电子排布规则进行调整
  // 这里使用简化的壳层模型
  return adjustShells(atomicNumber)
}

// 根据原子序数返回正确的电子壳层分布
function adjustShells(atomicNumber: number): number[] {
  const shellDistributions: Record<number, number[]> = {
    1: [1],
    2: [2],
    3: [2, 1],
    4: [2, 2],
    5: [2, 3],
    6: [2, 4],
    7: [2, 5],
    8: [2, 6],
    9: [2, 7],
    10: [2, 8],
    11: [2, 8, 1],
    12: [2, 8, 2],
    13: [2, 8, 3],
    14: [2, 8, 4],
    15: [2, 8, 5],
    16: [2, 8, 6],
    17: [2, 8, 7],
    18: [2, 8, 8],
    19: [2, 8, 8, 1],
    20: [2, 8, 8, 2],
    21: [2, 8, 9, 2],
    22: [2, 8, 10, 2],
    23: [2, 8, 11, 2],
    24: [2, 8, 13, 1],
    25: [2, 8, 13, 2],
    26: [2, 8, 14, 2],
    27: [2, 8, 15, 2],
    28: [2, 8, 16, 2],
    29: [2, 8, 18, 1],
    30: [2, 8, 18, 2],
    31: [2, 8, 18, 3],
    32: [2, 8, 18, 4],
    33: [2, 8, 18, 5],
    34: [2, 8, 18, 6],
    35: [2, 8, 18, 7],
    36: [2, 8, 18, 8],
    37: [2, 8, 18, 8, 1],
    38: [2, 8, 18, 8, 2],
    39: [2, 8, 18, 9, 2],
    40: [2, 8, 18, 10, 2],
    41: [2, 8, 18, 12, 1],
    42: [2, 8, 18, 13, 1],
    43: [2, 8, 18, 13, 2],
    44: [2, 8, 18, 15, 1],
    45: [2, 8, 18, 16, 1],
    46: [2, 8, 18, 18],
    47: [2, 8, 18, 18, 1],
    48: [2, 8, 18, 18, 2],
    49: [2, 8, 18, 18, 3],
    50: [2, 8, 18, 18, 4],
    51: [2, 8, 18, 18, 5],
    52: [2, 8, 18, 18, 6],
    53: [2, 8, 18, 18, 7],
    54: [2, 8, 18, 18, 8],
    55: [2, 8, 18, 18, 8, 1],
    56: [2, 8, 18, 18, 8, 2],
    // 镧系元素 57-71
    57: [2, 8, 18, 18, 9, 2],
    58: [2, 8, 18, 19, 9, 2],
    59: [2, 8, 18, 21, 8, 2],
    60: [2, 8, 18, 22, 8, 2],
    61: [2, 8, 18, 23, 8, 2],
    62: [2, 8, 18, 24, 8, 2],
    63: [2, 8, 18, 25, 8, 2],
    64: [2, 8, 18, 25, 9, 2],
    65: [2, 8, 18, 27, 8, 2],
    66: [2, 8, 18, 28, 8, 2],
    67: [2, 8, 18, 29, 8, 2],
    68: [2, 8, 18, 30, 8, 2],
    69: [2, 8, 18, 31, 8, 2],
    70: [2, 8, 18, 32, 8, 2],
    71: [2, 8, 18, 32, 9, 2],
    72: [2, 8, 18, 32, 10, 2],
    73: [2, 8, 18, 32, 11, 2],
    74: [2, 8, 18, 32, 12, 2],
    75: [2, 8, 18, 32, 13, 2],
    76: [2, 8, 18, 32, 14, 2],
    77: [2, 8, 18, 32, 15, 2],
    78: [2, 8, 18, 32, 17, 1],
    79: [2, 8, 18, 32, 18, 1],
    80: [2, 8, 18, 32, 18, 2],
    81: [2, 8, 18, 32, 18, 3],
    82: [2, 8, 18, 32, 18, 4],
    83: [2, 8, 18, 32, 18, 5],
    84: [2, 8, 18, 32, 18, 6],
    85: [2, 8, 18, 32, 18, 7],
    86: [2, 8, 18, 32, 18, 8],
    87: [2, 8, 18, 32, 18, 8, 1],
    88: [2, 8, 18, 32, 18, 8, 2],
    // 锕系元素 89-103
    89: [2, 8, 18, 32, 18, 9, 2],
    90: [2, 8, 18, 32, 18, 10, 2],
    91: [2, 8, 18, 32, 20, 9, 2],
    92: [2, 8, 18, 32, 21, 9, 2],
    93: [2, 8, 18, 32, 22, 9, 2],
    94: [2, 8, 18, 32, 24, 8, 2],
    95: [2, 8, 18, 32, 25, 8, 2],
    96: [2, 8, 18, 32, 25, 9, 2],
    97: [2, 8, 18, 32, 27, 8, 2],
    98: [2, 8, 18, 32, 28, 8, 2],
    99: [2, 8, 18, 32, 29, 8, 2],
    100: [2, 8, 18, 32, 30, 8, 2],
    101: [2, 8, 18, 32, 31, 8, 2],
    102: [2, 8, 18, 32, 32, 8, 2],
    103: [2, 8, 18, 32, 32, 8, 3],
    104: [2, 8, 18, 32, 32, 10, 2],
    105: [2, 8, 18, 32, 32, 11, 2],
    106: [2, 8, 18, 32, 32, 12, 2],
    107: [2, 8, 18, 32, 32, 13, 2],
    108: [2, 8, 18, 32, 32, 14, 2],
    109: [2, 8, 18, 32, 32, 15, 2],
    110: [2, 8, 18, 32, 32, 16, 2],
    111: [2, 8, 18, 32, 32, 17, 2],
    112: [2, 8, 18, 32, 32, 18, 2],
    113: [2, 8, 18, 32, 32, 18, 3],
    114: [2, 8, 18, 32, 32, 18, 4],
    115: [2, 8, 18, 32, 32, 18, 5],
    116: [2, 8, 18, 32, 32, 18, 6],
    117: [2, 8, 18, 32, 32, 18, 7],
    118: [2, 8, 18, 32, 32, 18, 8],
  }

  return shellDistributions[atomicNumber] || [atomicNumber]
}

const shellNames = ["K", "L", "M", "N", "O", "P", "Q"]

export function ElectronShell({ element }: ElectronShellProps) {
  const [mounted, setMounted] = useState(false)
  const shells = getElectronShells(element.atomicNumber)
  const category = categoryInfo[element.category]

  useEffect(() => {
    setMounted(true)
  }, [])

   // 计算每个壳层的半径和电子位置
   const baseRadius = 30
   const radiusIncrement = 18
   const centerX = 120
   const centerY = 120

  // 生成电子位置
  const generateElectronPositions = (shellIndex: number, electronCount: number) => {
    const radius = baseRadius + shellIndex * radiusIncrement
    const positions: { x: number; y: number; delay: number }[] = []
    
    for (let i = 0; i < electronCount; i++) {
      const angle = (2 * Math.PI * i) / electronCount - Math.PI / 2
      positions.push({
        x: centerX + radius * Math.cos(angle),
        y: centerY + radius * Math.sin(angle),
        delay: i * 0.1 + shellIndex * 0.2,
      })
    }
    return positions
  }

  // 获取元素的主色调
  const getColor = () => {
    const colorMap: Record<string, string> = {
      "bg-red-500/80": "#ef4444",
      "bg-orange-500/80": "#f97316",
      "bg-yellow-500/80": "#eab308",
      "bg-green-500/80": "#22c55e",
      "bg-teal-500/80": "#14b8a6",
      "bg-cyan-500/80": "#06b6d4",
      "bg-blue-500/80": "#3b82f6",
      "bg-indigo-500/80": "#6366f1",
      "bg-pink-500/80": "#ec4899",
      "bg-rose-500/80": "#f43f5e",
    }
    return colorMap[category.color] || "#3b82f6"
  }

  const primaryColor = getColor()

  return (
    <div className="flex flex-col items-center">
      {/* 原子模型 SVG */}
      <div className="relative">
         <svg width="240" height="240" viewBox="0 0 240 240" className="overflow-visible">
          {/* 背景光晕 */}
          <defs>
            <radialGradient id={`nucleus-glow-${element.atomicNumber}`} cx="50%" cy="50%" r="50%">
              <stop offset="0%" stopColor={primaryColor} stopOpacity="0.4" />
              <stop offset="100%" stopColor={primaryColor} stopOpacity="0" />
            </radialGradient>
            <filter id="electron-glow">
              <feGaussianBlur stdDeviation="2" result="blur" />
              <feMerge>
                <feMergeNode in="blur" />
                <feMergeNode in="SourceGraphic" />
              </feMerge>
            </filter>
          </defs>

          {/* 背景光晕效果 */}
          <circle
            cx={centerX}
            cy={centerY}
            r={baseRadius + shells.length * radiusIncrement + 10}
            fill={`url(#nucleus-glow-${element.atomicNumber})`}
          />

          {/* 电子壳层轨道 */}
          {shells.map((_, index) => {
            const radius = baseRadius + index * radiusIncrement
            return (
              <circle
                key={`orbit-${index}`}
                cx={centerX}
                cy={centerY}
                r={radius}
                fill="none"
                stroke="rgba(255,255,255,0.15)"
                strokeWidth="1"
                strokeDasharray="4 4"
                className={mounted ? "animate-pulse" : ""}
                style={{
                  animationDelay: `${index * 0.3}s`,
                  animationDuration: "3s",
                }}
              />
            )
          })}

           {/* 原子核 */}
           <circle
             cx={centerX}
             cy={centerY}
             r="16"
             fill={primaryColor}
             className={mounted ? "animate-pulse" : ""}
             style={{ animationDuration: "2s" }}
           />
           
           {/* 原子核内的元素符号 */}
           <text
             x={centerX}
             y={centerY}
             textAnchor="middle"
             dominantBaseline="central"
             fill="white"
             fontSize="9"
             fontWeight="bold"
             fontFamily="monospace"
           >
            {element.symbol}
          </text>

          {/* 电子 */}
          {mounted && shells.map((electronCount, shellIndex) => {
            const positions = generateElectronPositions(shellIndex, electronCount)
            const orbitRadius = baseRadius + shellIndex * radiusIncrement
            const animationDuration = 3 + shellIndex * 1.5 // 外层轨道旋转更慢

            return (
              <g
                key={`shell-${shellIndex}`}
                className="electron-shell-rotate"
                style={{
                  transformOrigin: `${centerX}px ${centerY}px`,
                  ["--duration" as string]: `${animationDuration}s`,
                  ["--direction" as string]: shellIndex % 2 === 0 ? "normal" : "reverse",
                } as React.CSSProperties}
              >
                {positions.map((pos, electronIndex) => (
                  <g key={`electron-${shellIndex}-${electronIndex}`}>
                    {/* 电子光晕 */}
                    <circle
                      cx={pos.x}
                      cy={pos.y}
                      r="6"
                      fill={primaryColor}
                      opacity="0.3"
                      filter="url(#electron-glow)"
                    />
                    {/* 电子核心 */}
                    <circle
                      cx={pos.x}
                      cy={pos.y}
                      r="3"
                      fill={primaryColor}
                      className="animate-pulse"
                      style={{
                        animationDelay: `${pos.delay}s`,
                        animationDuration: "1.5s",
                      }}
                    />
                    {/* 电子高光 */}
                    <circle
                      cx={pos.x - 1}
                      cy={pos.y - 1}
                      r="1"
                      fill="white"
                      opacity="0.8"
                    />
                  </g>
                ))}
              </g>
            )
          })}
        </svg>

      </div>

      {/* 电子壳层信息 */}
      <div className="mt-4 flex items-center justify-center gap-2 flex-wrap">
        {shells.map((electronCount, index) => (
          <div
            key={`shell-info-${index}`}
            className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-white/5 border border-white/10"
          >
            <span className="text-xs font-medium text-white/70">{shellNames[index]}</span>
            <span className="text-xs font-mono text-white">{electronCount}</span>
          </div>
        ))}
      </div>

      {/* 总电子数 */}
      <div className="mt-2 text-xs text-white/50">
        总电子数: <span className="text-white/80 font-mono">{element.atomicNumber}</span>
      </div>
    </div>
  )
}
